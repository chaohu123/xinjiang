...
## 阶段 3：成长体系与数据化（Week 5+）

### 7. 用户成长系统
- 数据库：`User` 表新增 `exp`, `level`, `badges`
- 成长规则文档（发帖、评论、收藏、报名等加权）
- 服务：经验值计算、等级判断、批处理/实时更新
- 前端：`LevelProgress.vue` 组件、`Profile.vue` 勋章展示、空状态

### 8. 文化足迹 & 成就证书
- 指标：浏览/收藏/路线/活动等数据汇总
- 后端服务：生成足迹统计、证书数据
- 前端：足迹仪表、证书预览 + PDF/PNG 导出
- 分享策略与权限控制
