...
export const getCarousels = () => {
  return request.get<CarouselItem[], CarouselItem[]>('/carousel')
}

// 获取所有轮播图（管理员）
export const getAllCarousels = () => {
  return request.get<CarouselItem[], CarouselItem[]>('/carousel/all')
}

// 创建轮播图
export const createCarousel = (data: CarouselCreateRequest) => {
  return request.post<CarouselItem, CarouselItem>('/carousel', data)
}

// 更新轮播图
export const updateCarousel = (id: number, data: CarouselUpdateRequest) => {
  return request.put<CarouselItem, CarouselItem>(`/carousel/${id}`, data)
}
...
export const uploadCarouselImage = (file: File) => {
  ...
  return request.post<{ url: string }, { url: string }>('/carousel/upload', formData, {
    headers: {
